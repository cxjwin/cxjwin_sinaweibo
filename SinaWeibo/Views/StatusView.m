...
- (void)retweetStatus {
    if (self.statusAction) {
		self.statusAction(self, nil, RetweetStatus);
	}
}
...
- (void)commentStatus {
    if (self.statusAction) {
		self.statusAction(self, nil, CommentStatus);
	}
}
...
- (void)praiseStatus {
    if (self.statusAction) {
		self.statusAction(self, nil, PraiseStatus);
	}
}
...
- (void)tapAvatar:(UITapGestureRecognizer *)tap {
	if (self.statusAction) {
		self.statusAction(self, nil, ShowUserInfo);
	}
}
...
- (void)tapImages:(UITapGestureRecognizer *)tap {
    if (self.statusAction) {
		self.statusAction(self, nil, ShowOriginalPic);
	}
}
...
- (void)touchedURLWithURLStr:(NSString *)urlStr {
	if (self.statusAction) {
		self.statusAction(self, urlStr, LinkToURL);
	}
}
