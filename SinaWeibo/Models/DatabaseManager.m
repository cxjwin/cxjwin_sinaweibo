...
static DatabaseManager *singleton = nil;
+ (DatabaseManager *)defaultManager {
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    singleton = [[DatabaseManager alloc] init];
  });
  return singleton;
}
...
- (int)checkAndCreateTables {
  ...
  for (NSString *name in self.tableNames) {
    ...
    if ([name isEqualToString:@"login_account"] && [result next] == NO) {
      [self createLoginAccountTable];
    }
    
    if ([name isEqualToString:@"statuses_data"] && [result next] == NO) {
...
- (int)createLoginAccountTable {
  ...
  char *sql = "create table login_account"
