...
@interface RequestUtilsTests : AsyncXCTestCase
...
- (void)testLoadStatus {
	...
	[OHHTTPStubs stubRequestsPassingTest:^BOOL(NSURLRequest *request) {
		...
		NSString *bundlePath = OHPathForFileInBundle(@"test_twenty_status.json", nil);

		OHHTTPStubsResponse *response = [OHHTTPStubsResponse responseWithFileAtPath:bundlePath
																		 statusCode:200
																			headers:@{@"Content-Type":@"text/json"}];
		
		[response requestTime:kRequestTime responseTime:kResponseTime];
		return response;
	}];
